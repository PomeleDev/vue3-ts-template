        ...
        >
          <template #default="{ data }">
            <p class="custom-item">
              ...
              <span
                ><el-button link @click="handleCreateChildMenu(data)">添加</el-button>
                <el-button link @click="handleRemoveMenu(data)">删除</el-button></span
              >
            </p>
          </template>
        </el-tree>
...
const allowDrag = (draggingNode: Node) => {
  return draggingNode.data.parent_id !== 0 || draggingNode.data.parent_id != null;
};
type DropType = "prev" | "inner" | "next";
const allowDrop = (draggingNode: Node, _dropNode: Node, type: DropType) => {
  if (draggingNode.data.parent_id === 0 || draggingNode.data.parent_id == null) {
    return type !== "inner";
  }
  return true;
};
